...
def gameOverScreen():
    ...
    text2 = font.render("Your score is: "+str(Score) , True, black)
    gameDisplay.blit(text2,(100,300))
    text3 = font.render("Click 't' to play again: " , True, black)
    ...
    text5 = font.render("Click 'e' to exit game " , True, black)
...
def animateNotes():
    ...
    global allNotes
    global Sprites
    #Here we are processing the sprite
    #I felt it was too slow so I did every frame twice
    images=[S_1,S_1,S_2,S_2,S_3,S_3,S_4,S_4,S_5,S_5,S_6,S_6]
    pygame.draw.rect(gameDisplay, black, [50,400,160,160])
    #moding by 12 would moving along the list
    gameDisplay.blit(images[Sprites%12],(50,400))
    Sprites+=1
    
    images_2=[SS_1,SS_2,SS_3,SS_4,SS_5,SS_6,SS_7,SS_8]
    pygame.draw.rect(gameDisplay, black, [0,100,200,200])
    #moding by 12 would moving along the list
    gameDisplay.blit(images_2[Sprites%8],(0,100))
...
########game keys
            ...
            #when a key is pressed, the game checks each note to see if it's within the region. If it is the user gets a point. The note is removed to show that it was accounted for.
                if event.key == pygame.K_s and Screen=="Game":
                    for anote in allNotes:
                        if anote.y>530 and anote.y<630 and anote.x==250:
                            ...
                            ScoreLabel(Score)
                if event.key == pygame.K_d and Screen=="Game":
                    for anote in allNotes:
                        if anote.y>530 and anote.y<630 and anote.x==350:
                            ...
                            allNotes.remove(anote)
                            ScoreLabel(Score)
                if event.key == pygame.K_j and Screen=="Game":
                    for anote in allNotes:
                        if anote.y>530 and anote.y<630 and anote.x==450:
                            ...
                            allNotes.remove(anote)
                            ScoreLabel(Score)
                if event.key == pygame.K_k and Screen=="Game":
                    for anote in allNotes:
